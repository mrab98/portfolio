    ...
    MODULE FEM1
          INTEGER(4),SAVE :: NPOIN,NELEM,NMATS,NBOUN,NPROP,NNODE,&
                             NEVAB,NSVAB,NDOFN,NDIME,NSTRE,NCASEE,NTYPE
      END MODULE FEM1
    ...
    MODULE FEM3
     ...
     REAL(8),ALLOCATABLE::U(:),UD(:),UDD(:),U2(:),U2D(:),U2DD(:),&
         &      KEFF(:,:),LEFF(:),DYNOLOAD(:,:),C(:,:) ,UC(:),&  
         &      FRE(:,:),AMP(:,:)           
     REAL(8),SAVE::T,DT,TTOT,TLOAD,DL,ALPHA,AC,BC  
    ...
    END MODULE FEM3
     ...
     MODULE FEM4
          INTEGER(4),SAVE :: NPOIN,NELEM,NBOUN,NMATS,NPROP,NNODE,&
                             NEVAB,NSVAB,NDOFN,NDIME,NSTRE,NCAS,AType,thick
      END MODULE FEM4
      
     MODULE FEM5

          Integer,Allocatable :: LNODS(:,:),MATNO(:),IFPRE(:),NTRAC(:),ELAREA(:)
    ...
    END MODULE FEM5
      ...
      MODULE FEM6
          INTEGER(4),SAVE :: NPOIN,NELEM,NBOUN,NMATS,NPROP,NNODE,&
                             NEVAB,NSVAB,NDOFN,NDIME,NSTRE,NCASE
      END MODULE FEM6
      
      MODULE FEM7
          INTEGER(4),ALLOCATABLE   :: LNODS(:,:),MATNO(:),IFPRE(:)
          REAL(8),ALLOCATABLE      :: PROPS(:,:),COORD(:,:),FIXED(:),& 
                                      RLOAD(:,:),ELOAD(:,:),ASTIF(:,:),&
                                      ASLOD(:),XDISP(:),TDISP(:,:),REACT(:),&
                                      TREAC(:,:),STRES(:,:)
      END MODULE FEM7
...
!     ****************************************************************
      ...
      Print*,"     1. Defining My Structure in BlackBox"
...
! ******************************************************************************************************************************
! ********************************************* FRAME AND TRUSS ANALYSIS ALGORITHM *********************************************       
   
      IF(ATYPE==1.or.ATYPE==2.or.ATYPE==3) THEN
          
      OPEN(7,FILE="IMPORT_FRAME.txt",STATUS="OLD",&
     	 FORM="FORMATTED",ACTION="READWRITE")            
      OPEN(8,FILE="EXPORT_FRAME.txt",STATUS="REPLACE",&
     	 FORM="FORMATTED",ACTION="READWRITE")   
      OPEN(1,FILE="TMP1.TMP",STATUS="SCRATCH",RECL=256,FORM="UNFORMATTED")
      OPEN(2,FILE="TMP2.TMP",STATUS="SCRATCH",RECL=256,FORM="UNFORMATTED")
      
	  CALL DATA_FRAME
      IF(ATYPE==1) CALL STIFF_1D
	  IF(ATYPE==2) CALL STIFF_2D
      IF(ATYPE==3) CALL STIFF_FRAME
      CALL ASSEMBA_FRAME
      DO ICASE=1,NCASE
      CALL LOADA_FRAME
      IF(ATYPE==3) CALL LOADB_FRAME
      CALL ASSEMBB_FRAME
      IF (ICASE==1) THEN
      CALL GREDUC_FRAME                                               
      ELSE
...
! ************************************************ DYNAMIC ANALYSIS ALGORITHM **************************************************       
  ...
  IF(ATYPE==5) THEN
              ...
              !*********************************** Truss Dynamic Analysis ***********************************
              
              
              IF(S==1) THEN
 
      OPEN(7,FILE="IMPORT_DYN_TRUSS.txt",STATUS="OLD",&
     	 FORM="FORMATTED",ACTION="READWRITE")            
      OPEN(8,FILE="EXPORT_DYN_TRUSS.txt",STATUS="REPLACE",&
     	 FORM="FORMATTED",ACTION="READWRITE") 
      ...
      STOP     
             ...
             !*********************************** Frame Dynamic Analysis ***********************************
              ...
              ELSE IF (S==2) THEN
                       OPEN(7,FILE="IMPORT_DYN_FRAME.txt",STATUS="OLD",&
     	 FORM="FORMATTED",ACTION="READWRITE")      
      OPEN(8,FILE="EXPORT_DYN_FRAME.txt",STATUS="REPLACE",&
     	 FORM="FORMATTED",ACTION="READWRITE")   
      OPEN(1,FILE="TMP.tmp",STATUS="SCRATCH",RECL=256,FORM="UNFORMATTED")
      OPEN(2,FILE="TMP2.tmp",STATUS="SCRATCH",RECL=256,FORM="UNFORMATTED")  
...
! ************************************************ CST ANALYSIS ALGORITHM   ****************************************************    
    ...
    END IF
     END IF         
      OPEN(7,FILE="IMPORT_CST.txt",STATUS="OLD",&
     	 FORM="FORMATTED",ACTION="READWRITE")    
      OPEN(8,FILE="EXPORT_CST.txt",STATUS="REPLACE",&
     	 FORM="FORMATTED",ACTION="READWRITE")  
...
!                                   
     ...
     Print*,"Number Of Properties (E/A/I/RO) : "; Read*,NPROP
     ...
     Print*,"Total Time: ";Read*,TTOT
     Print*,"Time Steps (Delta T)";Read*,DT
     Print*,"Load Type: 1.Cos  2.Sin";Read*,TLOAD
     Print*,"Delta Coefficient: ";Read*,DL
     Print*,"Alpha Coefficient: ";Read*,ALPHA
     Print*," "
     Print*," C= (alpha)M + (betta)K "
     Print*," Please Enter 'alpha' and 'betta' "
     Read*,AC
     Read*,BC
...
ELSE
      ...
      READ(7,*)  NPOIN,NELEM,NBOUN,NMATS,NPROP,NNODE,NDOFN,NDIME,& 
                 NSTRE,TTOT,DT,TLOAD,DL,ALPHA,AC,BC        
...
905                FORMAT(//1X,'NPOIN =',I5,3X,'NELEM =',I5,3X,'NBOUN =',I5,3X,'NMATS =',I5,3X,&
      ...
      Allocate(LNODS(NELEM,NNODE),MATNO(NELEM),IFPRE(NSVAB),&
        ...
        RLOAD(NPOIN,NDOFN),ELOAD(NELEM,NEVAB),ASTIF(NSVAB,NSVAB),C(NSVAB,NSVAB),&
        ASLOAD(NSVAB),XDISP(NSVAB),TDISP(NPOIN,NDOFN),REACT(NSVAB),&
        TREAC(NPOIN,NDOFN),STRESS(NELEM,NSTRE),ICODE(NDOFN),PRESC(NDOFN),&
        FACTOR(NSVAB,NSVAB),U(NSVAB),UD(NSVAB),UDD(NSVAB),U2(NSVAB),UC(NSVAB),&
...
950   FORMAT('MATERIAL PROPERTIES') 
      DO  IMATS=1,NMATS                  
          ...
          IF(Option==1) Then
          Print*," 1.Material's Type Label 2.E / 3.A / 4.I / 5.RO"
...
33   FORMAT('ELEMENT',5X,'NODES',18X,'MAT') 
      DO  IELEM=1,NELEM                                               
    IF(Option==1) Then
          Print*," 1.Element's Label / 2.Nodes / 3.Material Num "
...
111   FORMAT(5X,'NODE',5X,'COORD')
      DO  IPOIN=1,NPOIN                                               
          ...
          Print*," 1.Node's Label  2.Coordinations 'X' and 'Y' "
...
980   FORMAT(1X,'RESTRAINED NODES,FIXITY CODE AND PRESCRIBED VALUES')        
      IF (NBOUN/=0) THEN
          ...
          IF(Option==1) Then              
          Print*," 1.Label for Intended fixed Node  2.XIcode  3.XPresc  4.YIcode  5.YPresc"
...
77   FORMAT(5X,'NODE',5X,'X FREQUENCY',5X,'Y FREQUENCY',3X,'TETHA FREQUENCY',3X,'X AMPLITUDE',3X,'Y AMPLITUDE','TETHA AMPLITUDE')                           
     ...
     Print*,"======================================================================"
      ...
      DO WHILE (IPOIN < NPOIN)
          IF(Option==1) Then              
          Print*,"1.Number of Node  2.Load's Frequency  3.Load's Amplitude"
          Print*," "
          Print*,"                *Attention* Define 2 and 3 for each DOF"
                      READ(*,*) IPOIN,FRE(IPOIN,:),AMP(IPOIN,:) 
          Else
...
!     ************************************************************* 
      SUBROUTINE PARAMETERSFRAME
        ...
        DO I=1,NSVAB
            DO J=1,NSVAB
            KEFF(I,J)=ASTIF(I,J)+A0*M(I,J)+A1*C(I,J)
            END DO
        END DO
        RETURN
        END
...
!     *************************************************************     
        ...
        UC=A1*U+A4*UD+A5*UDD
        LP=0.0
        DO I=1,NSVAB
            DO J=1,NSVAB
                LP=LP+M(I,J)*UX(J)+C(I,J)*UC(J) 
            END DO
            LEFF(I)= ASLOAD(I) + LP
            LP=0
...
!     *******************************************************************
     ...
     Print*,"                             RESULTS"
     Print*,"======================================================================"
        PRINT*,"    TIME               NUMBER OF DOF    DISPLACEMENT(U(X))"
        WRITE(8,10)
...
11          FORMAT(F6.2,'|',3X,I3,10X,'|',E15.4)           
        ...
        WRITE(8,13)
                ...
                IF (T==TTOT) THEN
                          Print*," "
      Print*," "
      Print*,"Special Thanks to: Prof. Attarnejad"
      Print*,"                     Mr. Abyar"
      Print*," "
      Print*," "
      END IF
...
905   FORMAT(//1X,'NPOIN =',I5,3X,'NELEM =',I5,3X,'NBOUN =',I5,3X,&
      ...
      Allocate(PROPS(NMATS,NPROP),COORD(NPOIN,NDIME),FIXED(NSVAB),RLOAD(NPOIN,NDOFN),ASLOD(NSVAB),XDISP(NSVAB),REACT(NSVAB),PRESC(NSVAB),ELOAD(NELEM,NEVAB),NTRAC(NELEM),ELAREA(NELEM))
...
950   FORMAT('MATERIAL PROPERTIES') 
      DO  IMATS=1,NMATS   
          ...
          IF(Option==1) Then
          Print*," 1.Material's Type Label 2.E   / 3.V   / 4.Body Force(X) 5.Body Force(Y)"
          READ(*,*)  JMATS,PROPS(JMATS,:)             
...
960   FORMAT('0',2X,'ELEMENT',3X,'NODES',3X,'MAT.') 
      DO  IELEM=1,NELEM               
          ...
          IF(Option==1) Then
          Print*," 1.Element's Label / 2.Nodes / 3.Material Num / 4.Tractions Num per Element"
          READ(*,*) JELEM,LNODS(JELEM,:),MATNO(JELEM),NTRAC(JELEM)
          
          Else
          READ(7,*)  JELEM,LNODS(JELEM,:),MATNO(JELEM),NTRAC(JELEM)
          End IF
          
          WRITE(8,920) JELEM,LNODS(JELEM,:),MATNO(JELEM),NTRAC(JELEM)
920       FORMAT(5I5)
      END DO
...
!     *****************************************************************
      ...
      Do IELEM=1,NELEM
          Body_Force=0.0
          ELEMLOAD(:,:)=0.0
  ! ******************* Calculating Body Loads *******************
          
          LPROP=MATNO(IELEM)                                                
          fx=PROPS(LPROP,3)                                              
          fy=PROPS(LPROP,4)   
     
            Body_Force(IELEM,1) = (thick*(ELAREA(IELEM))/3)*fx
            Body_Force(IELEM,2) = (thick*(ELAREA(IELEM))/3)*fy
            Body_Force(IELEM,3) = (thick*(ELAREA(IELEM))/3)*fx
            Body_Force(IELEM,4) = (thick*(ELAREA(IELEM))/3)*fy
            Body_Force(IELEM,5) = (thick*(ELAREA(IELEM))/3)*fx
            Body_Force(IELEM,6) = (thick*(ELAREA(IELEM))/3)*fy

    
! ******************* Defining and Calculating Traction Loads  *******************            
    
  DO ITRAC=1,NTRAC(IELEM)

     
          IF(Option==1) Then
              
     Print*,"======================================================================"
     Print*,"                        Defining Elemental Loading"
     Print*,"======================================================================"
          Print*," "
          Print*,"Please Define Traction for Element Number: ",IELEM
          Print*,"1.Node1  2.Node2  3.Node3"
          Print*,"*** Traction is on Node1-Node2 Side" !Node3 Won't be affected
          ...
          WRITE(8,*) " "
          WRITE(8,*) " "
          WRITE(8,*) "       NODE1       NODE2    TX1             TX2          TY1            "
          WRITE(8,*) " "
          WRITE(8,*) NODE1,NODE2,Tx1,Tx2,Ty1
          WRITE(8,*) "TY2"
          WRITE(8,*) Ty2
...
ELOAD(IELEM,2*(NODE3))=0
          ...
          ELOAD(IELEM,:) = ELEMLOAD(IELEM,:) + ELOAD(IELEM,:)
          ELEMLOAD(IELEM,:) = ELOAD(IELEM,:)
          
                        
  END DO
  
  ELOAD(IELEM,:) =  ELOAD(IELEM,:) + Body_Force(IELEM,:)
...
!     ***************************************************************	
	 ...
	 USE FEM4
      ...
      DO  IELEM=1,NELEM   !LOOP OVER EACH ELEMENT       
          ...
          ELAREA(IELEM)=SQRT(SEMIPERIMETER*(SEMIPERIMETER-ELENG3)*(SEMIPERIMETER-ELENG2)*(SEMIPERIMETER-ELENG1))
          
          VALUE1=YOUNG/(1-V*V)
          VALUE2=YOUNG/((1+V)*(1-2*V))
          VALUE3=(2*(ELAREA(IELEM)))
          VALUE4=1/VALUE3
          ...
          MULTI=MATMUL(B,D)
          ESTIF=thick*(ELAREA(IELEM))*MATMUL(MULTI,TRANSPOSE(B))
...
!920       FORMAT(1X,'ELEMENT',6X,'STRESSES')                            
      ...
      Print*," "
      Print*," "
      Print*,"Special Thanks to: Professor Attarnejad"
      Print*,"                         Mr. Abyar"
      Print*," "
      Print*," "
      Pause
...
!                                    
     ...
     Print*,"Dimension of Structure: "; Read*,NDIME
     Print*,"Number Of Force Components in Each Point: "; Read*,NSTRE   
     Print*,"Number Of Load Cases: ";Read*,NCASE
...
END IF
      ...
      WRITE(8,905) NPOIN,NELEM,NBOUN,NMATS,NPROP,NNODE,NDOFN,NDIME,&
                   NSTRE,NCASE                                                 
905   FORMAT(//1X,'NPOIN =',I5,3X,'NELEM =',I5,3X,'NBOUN =',I5,3X,&
              'NMATS =',I5,3X,'NPROP =',I5,//1X,'NNODE =',I5,3X&
              'NDOFN =',I5,3X,'NDIME =',I5,3X,'NSTRE =',I5,3X,'NCASE =',I5)                   
      ...
      NEVAB=NNODE*NDOFN      
         ...
         ALLOCATE(PROPS(NMATS,NPROP),COORD(NPOIN,NDIME),FIXED(NSVAB),&
                  RLOAD(NPOIN,NDOFN),ELOAD(NELEM,NEVAB),&
                  ASTIF(NSVAB,NSVAB),ASLOD(NSVAB),XDISP(NSVAB),&
                  TDISP(NPOIN,NDOFN),REACT(NSVAB),TREAC(NPOIN,NDOFN),STRES(NELEM,NSTRE))
         
         ALLOCATE(ICODE(NDOFN),PRESC(NDOFN))
...
950   FORMAT('MATERIAL PROPERTIES') 
      DO  IMATS=1,NMATS                                               
          IF(Option==1) Then
          Print*," 1.Material's Type Label 2.E / 3.A / 4.I "
          READ(*,*)  JMATS,PROPS(JMATS,:)             
...
!                            
      ...
      WRITE(8,960)                                                      
960   FORMAT('0',2X,'ELEMENT',3X,'NODES',4X,'MAT.') 
      DO  IELEM=1,NELEM                                               
    IF(Option==1) Then
          Print*," 1.Element's Label / 2.Nodes / 3.Material Num "
          READ(*,*) JELEM,LNODS(JELEM,:),MATNO(JELEM)
...
!     ***************************************************************	
      ...
      USE FEM7
      REAL(8) :: KOUNT
      REAL(8),ALLOCATABLE :: ESTIF(:,:),TRANS(:,:)
              ALLOCATE(ESTIF(NEVAB,NEVAB),TRANS(NEVAB,NEVAB))
!
!     EVALUATION OF MEMBER STIFFNESS MATRICES                           
!     FOR PIN JOINTED PLANE FRAMEWORKS
      ...
      DO  IELEM=1,NELEM   !LOOP OVER EACH ELEMENT                                            
          ...
          ESTIF(1,2:3)=0
          ESTIF(2:3,1)=0
          ESTIF(2,2)=12*FEI/(ELENG**3)
          ESTIF(2,3)=6*FEI/(ELENG**2)                                     
          ESTIF(3,2)=6*FEI/(ELENG**2)
          ESTIF(3,3)=4*FEI/ELENG
          ...
          ESTIF=MATMUL(TRANS,MATMUL(ESTIF,TRANSPOSE(TRANS)))
          
	      DO  INODE=1,NNODE                                               
                  DO  JNODE=1,NNODE                                                                                    
                      DO  KNODE=1,NNODE+1                                               
                          DO  LNODE=1,NNODE+1   
                              INDEX=(INODE-1)*(NNODE+1)+KNODE                                       
                              JNDEX=(JNODE-1)*(NNODE+1)+LNODE
                              SELECT CASE(KNODE*LNODE)
                               CASE(1,4)
                                KOUNT=(-1)**(INDEX+JNDEX)
                               CASE(9)
                                SELECT CASE((-1)**(INDEX+JNDEX))
                                CASE(1)    
                                KOUNT=1    
                                CASE DEFAULT
                                KOUNT=.5
                               ...
                               CASE(2)
                                ...
                                IF (INDEX==4) THEN
                                KOUNT=-1
                                ELSE IF (JNDEX==4) THEN
                                KOUNT=-1
                                ELSE
                                KOUNT=1
                               END IF
                               END SELECT 
                              ESTIF(INDEX,JNDEX)=ESTIF(KNODE,LNODE)*KOUNT
                          END DO
                      END DO
                  END DO 
          END DO
...
!     ********************************************************************
!     SUBROUTINE LOADA:THIS SUBROUTINE IS USED TO
!   READ AND WRITE THE NODAL(JOINT) APPLIED LOADS FROM FILE NO.7
!     ********************************************************************
...
990   FORMAT(7X,'NODE',7X,'LOADS') 
     ...
     Print*,"======================================================================"
      ...
      DO WHILE (IPOIN < NPOIN)
          ...
          IF(Option==1) Then              
          Print*,"1.Node's Label  2.Load X  3.Load Y  4.Moment"
...
!     ********************************************************************
    SUBROUTINE LOADB_FRAME
      ...
      INTEGER(4) ::  CHDISTRIB,CHPOINT 
      REAL(8)    ::  DISTRIB(NELEM,NDOFN),POINT(NELEM,(2*NDOFN)),QLOAD(NPOIN,NDOFN),TQLOAD(NPOIN,NDOFN)
      
      WRITE(8,690)                                                      
690   FORMAT(7X,'ELEMENT',4X,'DISTRIBUTED LOADS(X,Y,M)',20X,'POINT LOADS(X-DX,Y-DY,M-DM)') 
     ...
     END IF
      ...
      DO WHILE (IELEM < NELEM)
            ...
            IF(Option==1) Then
                ...
                Print*,"1.Element's Label  2. Distributed(1 OR 0)   3. Concetrated(1 OR 0)"
                Print*," "
                Print*," *Attention* Please Enter 1 in case of Existence"
                READ(*,*) IELEM,CHDISTRIB,CHPOINT
                ELSE
            READ(7,*) IELEM,CHDISTRIB,CHPOINT 
                End IF
                
                IF(Option==1.and.CHDISTRIB==1) Then
                    
     Print*,"======================================================================"
     Print*,"                        Defining Distributed Loading"
     Print*,"======================================================================"
     Print*," "
                 Print*,"Please Enter Load in Each DOF"
                 Print*," 1. X  2.Y  3.M "
                 READ(*,*) DISTRIB(IELEM,:) 
                 
                ELSE IF(Option==1.and.CHPOINT==1) Then
                    
     Print*,"======================================================================"
     Print*,"                        Defining Concentrated Loading"
     Print*,"======================================================================"
     Print*," "
                 Print*,"For Defining Intended Concentrated Load:"
                 Print*,"Please Enter Load in Each DOF and Corresponding Distance From Left Node"
                 Print*," 1.Px  2.Dx  3.Py  4.Dy  5.M  6.Dm "
                 READ(*,*) POINT(IELEM,:)
                
                ELSE IF(Option.ne.1) Then
                
                IF (CHDISTRIB==1) READ(7,*) DISTRIB(IELEM,:)   
                IF (CHPOINT==1) READ(7,*) POINT(IELEM,:)
                
                END IF
                
            WRITE(8,630) IELEM,DISTRIB(IELEM,:),POINT(IELEM,:)
630         FORMAT(I10,3F15.5,6F15.5)           
      ...
      END DO     
       ...
       TQLOAD(:,:)=0.0    
        ...
        QLOAD(NODE1,1)=-((-DISTRIB(IELEM,1)*ELENG/2)+(-POINT(IELEM,1)*((ELENG-POINT(IELEM,2))**2)*(ELENG+2*POINT(IELEM,2))/((ELENG)**3)))
        QLOAD(NODE2,1)=-((-DISTRIB(IELEM,1)*ELENG/2)+(-POINT(IELEM,1)*((POINT(IELEM,2))**2)*(3*ELENG-2*POINT(IELEM,2))/((ELENG)**3)))
        QLOAD(NODE1,2)=-((-DISTRIB(IELEM,2)*ELENG/2)+(-POINT(IELEM,3)*((ELENG-POINT(IELEM,4))**2)*(ELENG+2*POINT(IELEM,4))/((ELENG)**3))&
                       +(-DISTRIB(IELEM,3)/4)+(6*POINT(IELEM,5)*POINT(IELEM,6)*(ELENG-POINT(IELEM,6))/((ELENG)**3)))
        QLOAD(NODE2,2)=-((-DISTRIB(IELEM,2)*ELENG/2)+(-POINT(IELEM,3)*((POINT(IELEM,4))**2)*(3*ELENG-2*POINT(IELEM,4))/((ELENG)**3))&
                       +(DISTRIB(IELEM,3)/4)+(-6*POINT(IELEM,5)*POINT(IELEM,6)*(ELENG-POINT(IELEM,6))/((ELENG)**3)))
        QLOAD(NODE1,3)=-((-DISTRIB(IELEM,2)*(ELENG**2)/12)+(-POINT(IELEM,3)*POINT(IELEM,4)*((ELENG-POINT(IELEM,4))**2)/(ELENG**2))&
                       +(-DISTRIB(IELEM,3)*ELENG/4)+(POINT(IELEM,5)*(ELENG-POINT(IELEM,6))*(3*POINT(IELEM,6)-ELENG)/(ELENG**2)))
        QLOAD(NODE2,3)=-((DISTRIB(IELEM,2)*(ELENG**2)/12)+(POINT(IELEM,3)*(POINT(IELEM,4)**2)*(ELENG-POINT(IELEM,4))/(ELENG**2))&
                       +(DISTRIB(IELEM,3)*ELENG/4)+(POINT(IELEM,5)*(POINT(IELEM,6))*(2*ELENG-3*POINT(IELEM,6))/(ELENG**2)))
        
        TQLOAD(NODE1,1)=COSTH*QLOAD(NODE1,1)-SINTH*QLOAD(NODE1,2)
        TQLOAD(NODE2,1)=COSTH*QLOAD(NODE2,1)-SINTH*QLOAD(NODE2,2)
        TQLOAD(NODE1,2)=SINTH*QLOAD(NODE1,1)+COSTH*QLOAD(NODE1,2)
        TQLOAD(NODE2,2)=SINTH*QLOAD(NODE2,1)+COSTH*QLOAD(NODE2,2)
        TQLOAD(NODE1,3)=QLOAD(NODE1,3)
        TQLOAD(NODE2,3)=QLOAD(NODE2,3)
        
       DO INODE=1,NNODE
           NODE=LNODS(IELEM,INODE) 
            DO  IDOFN=1,NDOFN
              NROWS=(INODE-1)*NDOFN+IDOFN                                      
              ELOAD(IELEM,NROWS)=ELOAD(IELEM,NROWS)+TQLOAD(NODE,IDOFN)
            END DO
...
!     *************************************************************
!     SUBROUTINE ASSEMB:IT ASSEMBLES THE ELEMENT LOADS MATRICES
!                       TO MAKE THE GLOBAL LOADS MATRIX
!     *************************************************************     
...
!     THIS ROUTINE ASSEMBLES APPLIED LOADS TO FORM THE                         
!     GLOBAL STIFFNESS MATRIX AND FORCE VECTOR                          
!                                                                                                                             
...
!     **************************************************************
      ...
      USE FEM6
      USE FEM7 
      REAL(8),ALLOCATABLE :: ESTIF(:,:),FOMEM(:) 
              ALLOCATE(ESTIF(NEVAB,NEVAB),FOMEM(NEVAB))
      REWIND 1                                                          
      DO  IELEM=1,NELEM ! LOOP OVER EAC ELEMENT
          READ(1) ESTIF
!                                                                       
...
900   FORMAT(5X,'NODE',1X,'DISPLACEMENTS',6X,'REACTIONS')
      ...
      DO  IPOIN=1,NPOIN                                               
          WRITE(8,910) IPOIN
          WRITE(8,975) TDISP(IPOIN,:)   
          WRITE(8,985) TREAC(IPOIN,:) 
          PRINT*,"         NODE            DISPLACEMENTS"
          PRINT*,IPOIN,TDISP(IPOIN,:)
          Print*,"       Reactions"
          Print*,TREAC(IPOIN,:)
          Print*,"------------------------------------------------------------------------"
      END DO
